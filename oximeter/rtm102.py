...
class work102:
    ...
    def socket_client(self, *args):
        ...
        while True:
            ...
            s.send(str.encode())
            self.socketFlag = 1
            self.js = json.loads(s.recv(3072))
            if self.js != []:
        ...
        while True:
            if self.socketFlag == 1:
                if self.Datalist != [] and kk < len(self.Datalist):
                    if self.Datalist[kk][1] > 120000:  # 判定手指有在上方
                        self.message = "◡̈)successful detection"
                        ...
                        w2 = dca2[0]
                        B.FindP(redacvalue)
                        self.ydata1 = self.ydata1[1:200] + [int(-A.rawData1)]  # smoothed ir value serial and invert signal
                        self.ydata2 = self.ydata2[1:200] + [int(-B.rawData1)]  # smoothed red value serial and invert signal
                        self.xdata1 = self.xdata1[1:200] + [self.Datalist[kk][2]]  # time serial
                        list = [0, 19, 39, 59, 79, 99, 119, 139, 159, 179, 199]
                        for i in list:
                            self.xdata[i] = self.xdata1[i]  # x-axis ticks
                        ...
                        if len(self.mx) >= 2:
                            for i in range(len(self.mx) - 1):
                                if (self.xdata1[self.mx[i + 1]] - self.xdata1[self.mx[i]]) != 0:
                                    self.hrate.append(int(60 / ((self.xdata1[self.mx[i + 1]] - self.xdata1[self.mx[i]]) / 1000)))
                        self.SPO2, SPO2_valid = calc_spo2(irraw, redraw, self.mx, len(self.mx))
                        self.SPO2=round(self.SPO2,1)
                        kk = kk + 1
                        text=str(self.mx)+";"+str(self.my)+";"+str(self.ydata1)+";"+str(self.ydata2)+";"+str(self.hrate)+";"+str(self.xdata)+";"+str(self.SPO2)+";"+str(self.message)
                        print("")
